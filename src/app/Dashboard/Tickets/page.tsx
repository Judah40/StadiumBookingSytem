...
const SignupSchema = Yup.object().shape({
  ...
  team2: Yup.string()
    ...
    .max(50, "Too Long!")
    .required("Required"),
  numberOfTicket: Yup.number().min(1, "Too few tickets").required("Required"),
...
function Tickets() {
  ...
  return (
    ...
    items-center bg-gray-900 bg-opacity-70 z-50 overflow-y-auto"
                ...
                tickets.map((item: any) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-4 py-3 border-b text-sm">
                      {item.normal_ticket_number}
                    </td>
                    <td className="px-4 py-3 border-b text-sm">
                      {item.vip_ticket_number}
                    </td>
                    <td className="px-4 py-3 border-b text-sm">{item.sold}</td>
                    <td className="px-4 py-3 border-b text-sm">{item.left}</td>
                    <td className="px-4 py-3 border-b text-sm">{item.date}</td>
                    <td className="px-4 py-3 border-b text-sm">
                      Le {item.vip_price} 
                    </td>
                    <td className="px-4 py-3 border-b text-sm">
                      Le {item.normal_price} 
                    </td>
                  </tr>
                ))}
            </tbody>
      ...
      <div className="w-32 h-12 mt-6 "></div>
    </DashboardLayout>
  );
}
